...
const SearchConfig: Schema<SearchConfig> = Schema.object({
    enabled: Schema.boolean().default(true).description('是否启用本指令'),
    minInterval: Schema.number().default(5000).description('指令触发的冷却时间 (毫秒)'),
...
const RouletteConfig: Schema<RouletteConfig> = Schema.intersect([
    Schema.object({
        enabled: Schema.boolean().default(true).description('是否启用本指令'),
        minInterval: Schema.number().default(0).description('指令触发的冷却时间 (毫秒)，默认 0 不限'),
        mode: Schema.union([
    ...
    Schema.union([
        ...
        Schema.object({
            ...
            initTime: Schema.number().min(0).default(30).description('每轮初始禁言时间（秒），无上限。决定空枪文案 / 新一轮文案中 `{time}` `{seconds}` 的起始值'),
            timeStepMin: Schema.number().min(0).default(15).description('每次空枪后叠加的随机最小时间（秒），无上限。影响空枪文案中 `{time}` `{seconds}`'),
            timeStepMax: Schema.number().min(0).default(120).description('每次空枪后叠加的随机最大时间（秒），无上限。影响空枪文案中 `{time}` `{seconds}`'),
        ...
        Schema.object({
            ...
            msgHit: Schema.string()
                ...
                .description('中弹文案。可用占位符：`{at}` 艾特触发用户 | `{time}` 禁言时长（格式化，取自 muteTime） | `{seconds}` 禁言秒数 | `{chamber}` 中弹时是第几枪 | `{total}` 弹轮总格数（取自 chambers）'),
            msgNewRound: Schema.string()
                .role('textarea')
...
const SleepConfig: Schema<SleepConfig> = Schema.object({
    enabled: Schema.boolean().default(true).description('是否启用本指令'),
    minInterval: Schema.number().default(0).description('指令触发的冷却时间 (毫秒)，默认 0 不限'),
    minSeconds: Schema.number().min(0).default(6 * 60 * 60).description('睡眠最短时长（秒），默认 6 小时，无上限。与 maxSeconds 共同决定占位符 `{time}` `{seconds}` 的随机区间下界'),
    maxSeconds: Schema.number().min(0).default(8 * 60 * 60).description('睡眠最长时长（秒），默认 8 小时，无上限。与 minSeconds 共同决定占位符 `{time}` `{seconds}` 的随机区间上界'),
...
const RepeatMuteConfig: Schema<RepeatMuteConfig> = Schema.object({
    enabled: Schema.boolean().default(true).description('是否启用本功能（关闭后不再监听群消息）'),
    guildMode: Schema.union([
        ...
        .default('all')
        .description('群生效范围模式'),
    guilds: Schema.array(Schema.string())
        .default([])
        .role('table')
        .description('群号列表（白名单 / 黑名单模式下使用；选择"所有群"时此处填写内容不生效但会保留）'),
    minTimes: Schema.number().min(1).default(5).description('触发禁言所需的最少重复次数。对应占位符 `{times}`'),
    muteSeconds: Schema.number()
        .min(1)
        .default(3600)
        .description('基础禁言时长（秒），无上限。最终禁言时长 = 此值 × 随机倍率。对应占位符 `{baseSeconds}`'),
    maxDurationMultiplier: Schema.number().min(1).default(8).description('随机禁言倍率的上限（1 ~ 该值随机取整数），无上限。对应占位符 `{multiplier}`'),
    msgMute: Schema.string()
        .role('textarea')
        .default('{at} 复读 {times} 次，被禁言 {time}！')
        .description(
            '触发禁言时的提示文案。可用占位符：`{at}` 艾特触发用户 | `{time}` 本次禁言时长（格式化，如 `01 小时 00 分 00 秒`） | `{seconds}` 本次禁言秒数 | `{times}` 触发所需重复次数（取自 minTimes） | `{baseSeconds}` 基础禁言秒数（取自 muteSeconds） | `{multiplier}` 本次随机倍率'
        ),
...
const SpamMuteConfig: Schema<SpamMuteConfig> = Schema.object({
    ...
    threshold: Schema.number().min(2).default(8).description('触发阈值：窗口内发送消息数达到该值即触发禁言。对应占位符 `{threshold}`'),
    muteSeconds: Schema.number().min(1).default(300).description('触发后禁言时长（秒），无上限。对应占位符 `{time}`（格式化后） / `{seconds}`（原始秒数）'),
    msgMute: Schema.string()
        .role('textarea')
        .default('{at} 刷屏了！{window} 秒内发送 {count} 条消息，禁言 {time} 冷静一下。')
        .description(
